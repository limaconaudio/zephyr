...
#define DEV_CFG(dev)                                            \
        ((struct uart_spinalhdl_config *) (dev)->config->config_info)
#define DEV_UART(dev)                                           \
        ((struct uart_spinalhdl_regs_t *)(DEV_CFG(dev))->uconf.base)
...
{
	volatile struct uart_spinalhdl_config *cfg = DEV_CFG(dev);
	volatile struct uart_spinalhdl_regs_t *uart = DEV_UART(dev);

	/* Config UART */
	uart->clk_div = ((SPINALHDL_FCLK_RATE / (cfg->baudrate * 8)) - 1) & 0xFFFFF;
...
static const struct uart_spinalhdl_config uart_spinalhdl_dev_cfg = {
	.uconf = {
		.base = (void *)SPINALHDL_UART_BASE_ADDR,
		.sys_clk_freq = CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC,
	},
	.baudrate = SPINALHDL_UART_BAUDRATE,
};
