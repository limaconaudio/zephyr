...
/**
 * @brief Platform Level Interrupt Controller (PLIC) driver for SPINALHDL SoC
 */
...
#define PLIC_SPINALHDL_IRQS        (CONFIG_NUM_IRQS - RISCV_MAX_GENERIC_IRQ)
#define PLIC_SPINALHDL_PRI_BITS	3
...
{
	u32_t key;
	u32_t spinalhdl_irq = irq - RISCV_MAX_GENERIC_IRQ;
	volatile u32_t *en =
		(volatile u32_t *)spinalhdl_PLIC_REG_IRQ_EN;

	key = irq_lock();
	*en |= (1 << (spinalhdl_irq - 1));
	irq_unlock(key);
}
...
{
	volatile u32_t *pri =
		(volatile u32_t *)spinalhdl_PLIC_REG_PRI;
	u32_t spinalhdl_irq;
	...
	if (priority > spinalhdl_PLIC_MAX_PRIORITY)
		priority = spinalhdl_PLIC_MAX_PRIORITY;

	spinalhdl_irq = irq - RISCV_MAX_GENERIC_IRQ;
	*pri |= (priority << ((spinalhdl_irq - 1) * 3));
}
...
static void PLIC_SPINALHDL_irq_handler(void *arg)
{
	volatile u32_t *id = (volatile u32_t *)spinalhdl_PLIC_REG_ID;
	u32_t irq;
	 ...
	 */
	if (irq == 0 || irq >= PLIC_SPINALHDL_IRQS)
		_irq_spurious(NULL);
...
/**
 *
 * @brief Initialize the SPINALHDL Platform Level Interrupt Controller
 * @return N/A
 */
static int PLIC_SPINALHDL_init(struct device *dev)
{
	ARG_UNUSED(dev);
	...
	volatile u32_t *pri_thres =
		(volatile u32_t *)spinalhdl_PLIC_REG_THRES;
	int i;
	...
	*pri = 0;
	for (i = 0; i < PLIC_SPINALHDL_IRQS; i++)
		*pri |= (1 << (i * PLIC_SPINALHDL_PRI_BITS));
...
SYS_INIT(PLIC_SPINALHDL_init, PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
